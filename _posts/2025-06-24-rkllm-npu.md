...
8. docker容器连wifi
    ...
    + frp服务
        ...
        ```
        接下来配置frp客户端。直接在网页上System-SOftware里下载luci-frpc-app然后在网页进行配置即可。

    + socat服务

      相比之下socat服务更简单，只需要来到iStoreOS来安装一下Socat。但是容器本身需要配置端口映射，将一些端口映射到容器外。此时如果宿主机本身就有公网IP的话，就可以将端口直接映射到域名+端口了。最后去网页iStore安装socat，通过网页接口设置端口转发功能。
